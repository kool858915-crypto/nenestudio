...
function buildRunnableToolFiles() {
  ...
  const scriptJs = [
    ...
    'const topicInput = document.querySelector("#tool-topic");',
    'const angleInput = document.querySelector("#tool-angle");',
    'const lengthSelect = document.querySelector("#tool-length");',
    ...
    "}",
    "",
    "function currentProvider() { return providerSelect?.value || config.provider || 'gemini'; }",
    "function currentApiKey() { return (apiKeyInput?.value || config.apiKey || '').trim(); }",
    "",
    ...
    "  if (!apiKey) { statusText.textContent = LABELS.statusNoKey; return; }",
    "  if (!topic && !selectedGenre) { statusText.textContent = LABELS.statusNoInput; return; }",
    "  persistKeyIfNeeded();",
    ...
    "  });",
    "  const data = await response.json().catch(() => ({}));",
    "  if (!response.ok) throw new Error(data.error?.message || 'OpenAI APIの実行に失敗しました。ブラウザ制限の場合は Gemini を試してください。');",
