...
function buildRunnableToolFiles() {
  ...
  const scriptJs = [
    ...
    "let providerSelect, apiKeyInput, rememberKey, topicInput, angleInput, lengthSelect, generateButton, statusText, resultBox, chipRow, bootTip;",
    "",
    ...
    "    if (statusText) setStatus(message);",
    ...
    "  } catch (error) {",
    "    setStatus(LABELS.error + (error.message || error));",
    "  }",
    "}",
    "",
    ...
    "  if (!apiKey) { setStatus(LABELS.statusNoKey); return; }",
    "  if (!topic && !selectedGenre) { setStatus(LABELS.statusNoInput); return; }",
    "  persistKeyIfNeeded();",
