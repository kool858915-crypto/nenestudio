...
function buildRunnableToolFiles() {
  ...
  const scriptJs = [
    ...
    "let providerSelect, apiKeyInput, rememberKey, topicInput, angleInput, lengthSelect, generateButton, statusText, resultBox, chipRow, bootTip, loadingBox, loadingTitle, loadingDetail;",
    ...
    "  }",
    "  if (loadingBox) loadingBox.hidden = !isBusy;",
    "  if (resultBox) resultBox.classList.toggle('is-loading', !!isBusy);",
    "  if (loadingTimer) {",
    ...
    "    selectGenre(first, { fillTopic: !(topicInput && topicInput.value.trim()) });",
    ...
    "    if (!button) return;",
    "    event.preventDefault();",
    "    const shouldFill = !topicInput || !topicInput.value.trim() || topicInput.dataset.filledByChip === '1';",
    "    selectGenre(button.getAttribute('data-topic') || '', { fillTopic: shouldFill });",
    "    setStatus((LABELS.promptTopic || 'テーマ:') + ' ' + ((topicInput && topicInput.value) || selectedGenre), '');",
    "  };",
    "  if (topicInput) {",
    "    topicInput.oninput = function () { topicInput.dataset.filledByChip = '0'; };",
    "  }",
    "}",
    "",
    ...
    "}",
    "",
    "function currentProvider() { return (providerSelect && providerSelect.value) || config.provider || 'gemini'; }",
    "function currentApiKey() { return ((apiKeyInput && apiKeyInput.value) || config.apiKey || '').trim(); }",
    "",
    ...
    "  if (!apiKey) { setStatus(LABELS.statusNoKey, 'is-error'); return; }",
    "  if (!topic && !selectedGenre) { setStatus(LABELS.statusNoInput, 'is-error'); return; }",
    "  persistKeyIfNeeded();",
    "  setBusy(true);",
    "  if (resultBox) resultBox.textContent = LABELS.generating || '作成中...';",
    "  const modeHints = [];",
    "  if (config.toolMode === 'stock_picker' || config.toolMode === 'crypto_picker') {",
    "    modeHints.push('必須: 注目リストの各項目に「選定理由」を必ず付ける');",
    "    modeHints.push('形式例: 【1】銘柄名 / 選定理由: ... / 注目度: 高 / 主なリスク: ...');",
    ...
    "    if (resultBox) resultBox.textContent = text || LABELS.empty;",
    "    setStatus(LABELS.complete, 'is-done');",
    "  } catch (error) {",
    "    if (resultBox && (!resultBox.textContent || resultBox.classList.contains('is-loading'))) {",
  ...
  if (mode === "stock_picker" || mode === "crypto_picker") {
    ...
    if (isEnglish) {
      return [
        ...
        `2. ${unit} shortlist (3-5 items). Every item MUST include a selection reason.`,
        "Format each item as:",
        ...
        "",
        "Do not output a name-only list without 選定理由.",
        investmentBan,
      ].join("\n");
    }
    return [
      ...
      "出力構成（日本語・この見出し名を守る）:",
      "1. 今日のテーマ要約（1〜2行）",
      `2. 注目${unit}リスト（3〜5件。各件に必ず選定理由を書く）`,
      "各件は次の形式で書く:",
      "【1】銘柄名（コードが分かるなら併記）",
